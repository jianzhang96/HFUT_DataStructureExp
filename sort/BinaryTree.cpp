...
void BinaryTree::set_elements(int a[],int n)
{
    elem.clear();
    elem.push_back(0);
    for(int i=1;i<n;i++)
...
void BinaryTree::creat_node(node*&pt,vector<int> t,int k)
{
    pt=new node;
    pt->left=pt->right=nullptr;
    pt->data=t[k];
    if(2*k<t.size()) creat_node(pt->left,t,2*k);
    if(2*k+1<t.size()) creat_node(pt->right,t,2*k+1);
}
void BinaryTree::update()
{
    clear(root);
    creat_node(root,elem,1);
}
//AVL树的方法
